...
deltact_384 <- function(dataframe,housekeeping_df, marker_gene, list_for_join){  
  #dataframe= a data frame, df1 from calctecrep.R
  #housekeeping_df=a data frame, df_hkg from pullhkg.R
  #marker_gene=a string, name for marker as appears in df1 primers column
  ...
  df_merge <- df_marker %>% #merge marker gene dataframe with housekeeping gene dataframe
    left_join(y=housekeeping_df, by=list_for_join) %>%  #example: by=c("Sample","genotype","treatment")
    relocate(Sample)
 
  df_dct <- df_merge %>% 
    mutate(d_cq=marker_cq-hkg_cq)
  
  return(list(df_dct, df_marker)) #list(df_dct, df_marker)#returns list of tables (1)is delta cq or delta ct. (2) is the isolated marker gene
}