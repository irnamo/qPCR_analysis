...
```
### Pull VIN3 (housekeeping gene)
```{r}
source("01_processing/pullhkg_384.R")

df_vin3 <- pullhkg_384(df1, "SlVIN3")
```
2024.01.22 - noticed high higher variability (23.5-36.4) in VIN3 compared to actin (17.9-26.9)

### calculate delta Ct --- for SlWAK1
```{r}
source("01_processing/deltact_384.R")

list_df2 <- deltact_384(df1,df_actin, "SlWAK1", c("Sample","genotype","treatment", "time"))
...
```{r}
bxp <- ggboxplot(
  df2_marker, x = "treatment", y = "d_cq", # I just realized that its all one genotype.... duh
...
```
```{r}
fig_2 <- ggboxplot(df_logdcq,x = "time",y = "log_d_cq",
                   order = (c("0","3","6","9","24")),
                   fill = "treatment", palette = "jco") +
   ylim(0,.6) + ylab("Relative transcript abundance") +
  ggtitle("WAK1") + theme(plot.title = element_text(hjust = 0.5))
...
file_name<-"20240123_a83_WT_WAK1_treatment" 
...
### figure 2
```{r}
fig_1 <- ggboxplot(df_logdcq,x = "treatment",y = "log_d_cq",
...
```{r}
folder_location<-"E:/Files/PhD Files 2 Iris Mollhoff/Git stuff/qPCR_analysis_flgII28/experiments/"
file_name<-"20240123_a83_WT_WAK1_time" 
##change file name but keep directory the same
