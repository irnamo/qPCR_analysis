...
```
```{r}
processeddatafolder <- "E:/Files/PhD Files 2 Iris Mollhoff/Git stuff/qPCR_analysis_flgII28/data/processed_data/"
write_csv(d_w, "E:/Files/PhD Files 2 Iris Mollhoff/Git stuff/qPCR_analysis_flgII28/data/processed_data/d_w.csv")
write_csv(d_a, "E:/Files/PhD Files 2 Iris Mollhoff/Git stuff/qPCR_analysis_flgII28/data/processed_data/d_a.csv")
```
...
```{r}
processeddatafolder <- "E:/Files/PhD Files 2 Iris Mollhoff/Git stuff/qPCR_analysis_flgII28/data/processed_data/"
path_file = paste(processeddatafolder, "df1.csv" ,sep = "")
write_csv(df1, path_file)
```
...
```{r}
processeddatafolder <- "E:/Files/PhD Files 2 Iris Mollhoff/Git stuff/qPCR_analysis_flgII28/data/processed_data/"
path_file = paste(processeddatafolder, "df_actin.csv" ,sep = "")
write_csv(df_actin, path_file)
```
...
df4_marker %>%
  ...
  shapiro_test(d_cq)
```
**2024.02.01 result** - for d_cq, samples that break normality assumption: ACET1-mock-24


###check qq-plot
```{r}
suppressWarnings(ggqqplot(df4_marker, "d_cq", ggtheme = theme_bw()) +
...
res.aov
```
**RESULT 2024.02.01** using d_cq: single effects for time, genotype, treatment. found 2-way effect by timeXtreatment, timeXgenotype. no 3-way effect between the three variables was found
...
two.way1.1 <- df4_marker %>%
  group_by(genotype) %>%
  anova_test(dv = d_cq, wid = rep, within = c(treatment, time))
two.way1.1
```
**RESULT 2024.01.30** error due to 12 bio replicate count instead of 3 bio reps
...
time.effect
```
**RESULT 2024.02.01** for d_cq, find significant time effect in all comparisons
...
pwc.r
```
**RESULT 2024.02.01** significant for WT-6h, ACET1-6h, ACET1-0h significant using delta ct
...
pwc.t
```
**RESULT 2024.02.01** multiple significant
...
pwc.g <- pwc.g %>% add_xy_position(x = "time")
pwc.g.filtered <- pwc.g %>% 
  filter(p.adj <1)

bxp1 + 
  stat_pvalue_manual(pwc.g.filtered, tip.length = 0) +
  labs(
    title = "Sl WAK1",
    subtitle = get_test_label(res.aov, detailed = TRUE),
    caption = get_pwc_label(pwc.g),
...
file_name<-"20240201_a83_WT-ACET1_WAK1_genotype_mix3xANOVA_dcq" 
...
###p-value modification on figure for TIME effect from delta cq
```{r}
# Visualization: box plots with p-values
...
```{r}
# Visualization: box plots with p-values
pwc.r <- pwc.r %>% add_x_position(x = "time")
pwc.r.filtered <- pwc.r %>% 
  filter(p.adj <1)

bxp2 + 
  stat_pvalue_manual(pwc.r.filtered, tip.length = 0, 
                     y.position =  0.55, bracket.shorten = .85) +
  labs(
    title = "Sl WAK1",
    subtitle = get_test_label(res.aov, detailed = TRUE),
    caption = get_pwc_label(pwc.r),
    x="Time (hrs)", y= "Relative Transcript Abundance / Sl Actin") +
...
```{r}
folder_location<-"E:/Files/PhD Files 2 Iris Mollhoff/Git stuff/qPCR_analysis_flgII28/experiments/"
file_name<-"20240201_a83_WT-ACET1_WAK1_treatment_mix3xANOVA_dcq" 
##change file name but keep directory the same
